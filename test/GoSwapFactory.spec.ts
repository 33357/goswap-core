...
import GoSwapPair from "../build/GoSwapPair.json";
...
describe("GoSwapFactory", () => {
  ...
  async function createPair(tokens: [string, string]) {
    const bytecode = `0x${GoSwapPair.evm.bytecode.object}`;
    ...
    await expect(factory.createPair(...tokens)).to.be.reverted; // GoSwap: PAIR_EXISTS
    await expect(factory.createPair(...tokens.slice().reverse())).to.be
      .reverted; // GoSwap: PAIR_EXISTS
  ...
  it("setFeeTo", async () => {
    ...
    ).to.be.revertedWith("GoSwap: FORBIDDEN");
